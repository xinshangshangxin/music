...
/******/ (() => {
  // webpackBootstrap
  /******/ "use strict";
  var __webpack_exports__ = {}; // CONCATENATED MODULE: ./src/lx.js
  ...
  const currentScript = currentScriptInfo
    ...
    : document.getElementsByTagName("script")[0].innerText; // CONCATENATED MODULE: ./src/apis/kw.js
  ...
  const parseCookieToken = (cookies) => {
    if (!cookies) return "";
    let cookieToken = Array.isArray(cookies)
      ? cookies.find((str) => str.startsWith("Hm_Iuvt_"))
      : cookies.match(/Hm_Iuvt_\w+=\w+;/)?.[0];
    ...
    new Promise((resolve, reject) => {
      ...
      request(
        ...
        async function (error, response) {
          ...
          const result = response.body.match(
            /https?:\/\/[/.\w]+\/kw-www\/\w+\.js/g
          );
          if (result) {
            const getAppToken = (url) =>
              new Promise((resolve) => {
                request(
                  url,
                  {
  ...
  const eapi = (url, object) => {
    ...
    return {
      params: buf2hex(
        aesEncrypt(data, eapiKey, "", "aes-128-ecb")
      ).toUpperCase(),
    };
  };
  ...
  const package_namespaceObject = /*#__PURE__*/ JSON.parse(
    '{"UU":"lx-music-source","rE":"1.1.2","cy":"lyswhut"}'
  ...
  const getVersion = async (index = 0) => {
    return getLatestVersion(address[index] + "/package.json")
      .then((version) => {
        return {
          version,
  ...
  const checkLatestVersion = async () => {
    ...
    return compareVersions(package_namespaceObject.rE, remoteVersion.version) <
      0
      ? remoteVersion
      : null;
