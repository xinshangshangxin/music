...
ENV NODE_ENV "docker"
RUN ./node_modules/.bin/tsc -p tsconfig.build.json && mkdir -p dist/environment/ && cp -r src/environment/docker.js dist/environment/docker.js
...
CMD ["pm2-runtime", "start", "index.js", "-n", "music-backend"]
